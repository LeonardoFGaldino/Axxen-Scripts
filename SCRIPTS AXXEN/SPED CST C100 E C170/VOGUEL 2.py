...
result_matrix = []
destination = list(range(len(availability)))  # Adicione esta linha para definir 'destination'
...
def main():
    ...
    while (sum_without_none(availability) + sum_without_none(need)) != 0:
        ...
        if value_need < value_availability:
            ...
            for i in range(0, len(cost_matrix)):
                cost_matrix[i][index_column_need] = 0
            ...
            for i in range(0, len(cost_matrix[0])):
                cost_matrix[index_line_availability][i] = 0
