...
def extrair_valores_cp(pdf_path):
    try:
        ...
        return cp_terceiros, cp_segurados, cp_patronal, IRRF, IRPJ, CSLL, PIS, COFINS, Valor_do_Pedido
...
for arquivo in os.listdir(pasta_pdf):
    if arquivo.endswith('.pdf'):
        ...
        cp_terceiros, cp_segurados, cp_patronal, IRRF, IRPJ, CSLL, PIS, COFINS, Valor_do_Pedido = extrair_valores_cp(pdf_path)
        informacoes_pdf.append((arquivo, cp_terceiros, cp_segurados, cp_patronal, IRRF, IRPJ, CSLL, PIS, COFINS, Valor_do_Pedido))

# Criar um DataFrame
df = pd.DataFrame(informacoes_pdf, columns=["Nome do Arquivo", "CP TERCEIROS", "CP SEGURADOS", "CP PATRONAL", "IRRF", "IRPJ", "CSLL", "PIS", "COFINS", "Valor_do_Pedido"])
