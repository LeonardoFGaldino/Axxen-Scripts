...
def extrair_valores_cnpj_razao_social_e_valor_total(pdf_path):
    try:
        ...
        # Escolhe a razão social baseado nos padrões encontrados
        razao_social = razao_social1 if razao_social1 else razao_social2

        # Padrão para extrair Valor Total
        valor_total_pattern = r'VALOR TOTAL\s*([\d.,]+)'
        match_valor_total = re.search(valor_total_pattern, texto)
        valor_total = match_valor_total.group(1).replace(',', '') if match_valor_total else '0.0'

        return cnpj, razao_social, float(valor_total.replace('.', '').replace(',', '.'))
